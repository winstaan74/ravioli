...
class ResourceTagLibIntegrationTests extends GroovyPagesTestCase {
	...
	void testResourceName() {
		...
		assertOutputEquals '<a class="res" href="/?q=ivo%3A%2F%2Ffoo.bar%2Fchoo">ivo://foo.bar/choo</a>', template, [body:' ivo://foo.bar/choo '] // no link attribute, but it looks like a url.		
		assertOutputEquals '<a class="res" href="/?q=ivo%3A%2F%2Ffoo.bar%2Fchoo">fred</a>', template, [uri:'ivo://foo.bar/choo', body:'fred'] // no link attribute, but it looks like a url.		
	...
	void testResourceNameXmlSlurper() {
		...
		assertOutputEquals '<a class="res" href="/?q=ivo%3A%2F%2FCDS">CDS</a>', template, [xml: xml.curation.publisher] // resource name with ivo-id
